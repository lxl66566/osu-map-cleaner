...
| `cs`    | 圆圈大小                                        |
| `star`  | nomod 星数（取难度自身模式的评分；osu! 未计算的谱面可用 `--calc-star` 本地补算） |
| `ar` / `od` / `hp` | AR / OD / HP                         |
...
# osu! 未写入星数的谱面（批量导入的 std 图）用 rosu-pp 本地补算后再过滤
...
| `--dry-run`               | 仅预览，不删除                                                                |
| `--calc-star`             | 用 rosu-pp 离线并行补算缺失的星数（lazer 算法，实测与 db 已有值偏差 <0.5 星） |
...
- `--calc-star` 补算只接受正数星值：空文件、损坏图、被 rosu-pp 判为可疑的图一律保持"未知"，星数条件永不匹配。
